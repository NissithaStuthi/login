...
def fetch_complaints():
    try:
        ...
        if response.status_code == 200:
            return response.json().get("complaints", [])
...
with tab_overview:
    ...
    if not df.empty and "priority" in df.columns:
        critical = int((df["priority"] == "P1").sum())
        high = int((df["priority"] == "P2").sum())
    else:
        critical = 0
        high = 0
    ...
    with m2:
        metric("Critical Cases", critical)

    with m3:
        metric("High Priority", high)

    with m4:
        metric("Open Cases", open_cases)
...
with tab_analyze:
    ...
    st.markdown(
        '<div class="page-title">Analyze Customer Complaint</div>',
        unsafe_allow_html=True,
    )
                ...
                if response.status_code == 200:
                    ...
                    with right:
                        ...
                        if recurring:
                            ...
                            st.warning(
                                f"Recurring issue detected: {recurring}"
                            )
